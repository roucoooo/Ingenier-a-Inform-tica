...
+!start : true <-
    .print("[MEDIUM] Iniciado. Capacidad: 30kg 1x2.");
    !!poll_entrance.
...
+!try_claim_container([cand(CId, CX, CY, CT) | Rest]) : state(idle) & autonomous_mode(none) <-
    ...
    if (container_info(CId, CW, CH, CWeight, CT, CX, CY, "pending") & CWeight <= 30.0 & CWeight > 10.0 & CW <= 1 & CH <= 2) {
        .broadcast(tell, claiming(CId)); .wait(600);
...
+!execute_store_task(CId, CX, CY) : state(idle) <-
    ...
    -+state(idle); .abolish(claimed_container(_)); .abolish(claimed_by_other(_));
    .broadcast(tell, robot_done(robot_medium, CId)); .send(supervisor, tell, stored(CId, ShelfId));
    !!poll_entrance.
...
+!select_best_shelf(CId, BestShelf) : true <-
    ...
    if (.length(Sorted) > 0) { .nth(0, Sorted, cand(_, BestShelf)); } else { BestShelf = "shelf_5"; };
...
+!find_and_deliver(Type) : state(idle) <-
    ...
    if (.length(Candidates) > 0) {
        .nth(0, Candidates, cand(CId, SId)); .broadcast(tell, claiming_exit(CId)); .wait(500);
...
+!deliver_to_exit(CId, SId) : state(idle) <-
    ...
    .send(scheduler, tell, container_delivered_ok(CId, "any")); .send(supervisor, tell, container_delivered_exit(CId, robot_medium));
...
+!navigate_adjacent(CX, CY) : true <-
    !try_adjacent_list(CX, CY, [ pos(CX-1, CY), pos(CX+1, CY), pos(CX, CY+1), pos(CX, CY-1), pos(CX, CY+2) ]).
...
+!try_adjacent_list(CX, CY, []) : true <- 
    .print("⏳ Medio atascado. Esperando turno...");
    .wait(2000); 

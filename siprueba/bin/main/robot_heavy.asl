...
+!start : true <-
    .print("[HEAVY] Iniciado. Capacidad: 100kg.");
    !!poll_entrance.
...
+!try_claim_container([cand(CId, CX, CY, CT) | Rest]) : state(idle) & autonomous_mode(none) <-
    ...
    if (container_info(CId, CW, CH, CWeight, CT, CX, CY, "pending") & (CWeight > 30.0 | CT == "urgent") & CW <= 2 & CH <= 3) {
        .broadcast(tell, claiming(CId)); .wait(700);
...
+!execute_store_task(CId, CX, CY) : state(idle) <-
    ...
    .broadcast(tell, robot_done(robot_heavy, CId)); .send(supervisor, tell, stored(CId, ShelfId));
    !!poll_entrance.
...
+!select_best_shelf(CId, BestShelf) : true <-
    ...
    if (.length(Sorted) > 0) { .nth(0, Sorted, cand(_, BestShelf)); } else { BestShelf = "shelf_8"; };
...
+!find_and_deliver(Type) : state(idle) <-
    ...
    if (.length(Candidates) > 0) {
        .nth(0, Candidates, cand(CId, SId)); .broadcast(tell, claiming_exit(CId)); .wait(600);
...
+!deliver_to_exit(CId, SId) : state(idle) <-
    ...
    .send(scheduler, tell, container_delivered_ok(CId, "any")); .send(supervisor, tell, container_delivered_exit(CId, robot_heavy));
...
+!try_adjacent_list(CX, CY, []) : true <- 
    .print("⏳ Pesado atascado. Esperando turno...");
    .wait(2500); 
